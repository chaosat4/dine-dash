...
import { Plus, Download, Printer, Trash2, QrCode, Copy, ExternalLink } from 'lucide-react'
    ...
    try {
      ...
      try {
        const res = await fetch('/api/tables', {
          ...
          body: JSON.stringify({ number: tableNum }),
        })
        if (res.ok) {
          const created = await res.json()
          newTable.id = created.id
        }
      } catch {
        // Demo mode
      }
  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text)]">QR Code Management</h1>
          <p className="text-gray-500">Create and manage table QR codes</p>
        </div>
        <div className="flex gap-3">
          <Button onClick={printAllQRs} variant="outline" className="no-print">
            <Printer className="w-4 h-4 mr-2" />
            Print All
          </Button>
          <Button onClick={() => setShowAddModal(true)} className="no-print">
      ...
      <div ref={printRef} className="hidden print:block">
        ...
        {selectedTable ? (
          <div className="p-8 text-center">
            <h1 className="text-3xl font-bold mb-2">Dine & Dash</h1>
            <p className="text-lg text-gray-600 mb-8">Scan to Order</p>
